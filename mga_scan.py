# -*- coding: utf-8 -*-
import itertools                                                          # 序列组合枚举
import math                                                               # 开方/π 等数学工具
import time                                                               # 计时
import sys                                                                # stderr / tty 检测
import os                                                                 # 终端宽度检测
import zlib                                                               # 序列名→种子(备用)
import pygmo as pg                                                        # 优化算法(SADE/COMPASS)
from pykep import epoch                                                   # 时间历元(mjd2000)
from pykep.planet import jpl_lp                                           # JPL 低精度行星星历
from pykep.trajopt import mga_1dsm                                        # MGA-1DSM 轨迹模型
from multiprocessing import Pool, cpu_count                               # 序列级并行

# ============================== 全局参数 ==============================
SADE_GEN = 1500                 # SADE 每次进化代数(全局搜索主力)
SCAN_SEEDS = [1, 2, 3]          # 扫描阶段每序列独立重启种子(取最优, 保证公平)
RESTART_SEEDS = [1, 2, 3, 4, 5] # 精算阶段 Top-N 序列使用的种子
TOP_N_REFINE = 10               # 进入精算的前排序列数
COMPASS_FEVALS = 5000           # COMPASS 局部精修最大评估次数
RESONANCE_HARMONICS = (1, 1.5, 2, 2.5, 3, 4)  # 谐波含半整数(冠军解曾自发找到3:2共振)
RESONANCE_BASE_TOL = 15.0       # 共振窗口基础容差(天)
RESONANCE_MODE = 'off'          # 'off'=共振段自由 | 'hard'=共振剪枝 | 'soft'=软罚(专项研究用)
RESONANCE_SOFT_WEIGHT = 30.0    # soft 模式: 每偏离共振窗口 1 天罚 30 m/s
...
T0_START_MJD2000 = 9131.0       # 发射窗口起点 ≈2025-01-01
T0_END_MJD2000 = 12784.0        # 发射窗口终点 ≈2035-01-01

# —— ΔV 计费口径(本版核心改动) ——
ARRIVAL_MODE = 'capture'        # 'capture'=到达按JOI捕获烧计费(新) | 'full'=到达v∞全额计费(旧)
CHARGE_DEPARTURE = True         # True: 出发v∞计入总ΔV(报表单列"火箭承担") | False: 完全交给火箭
DEPARTURE_VINF_MAX = 5.6        # 出发超速上限 km/s ≈ C3 31.4 (朱诺实况, 运载能力边界)
JOI_RP_RADII = 6.0              # JOI 近木点(木星半径倍数; 6R_J 避开辐射带)
JOI_PERIOD_DAYS = 300.0         # JOI 捕获目标轨道周期(天, 大椭圆泵下轨道第一站)
...
def generate_sequences():
    """枚举 E→(0~4个中间行星)→J 全部序列, 中间行星 ∈ {E,V,M}"""
    planets = ['earth', 'venus', 'mars']
    all_seqs = []
    for r in range(0, 5):                                                     # 中间行星个数
        for combo in itertools.product(planets, repeat=r):
...
def get_vinf_bounds(seq_names):
    """发射 v∞ 包络(km/s): 按借力次数给, 并统一按运载火箭能力封顶"""
...
def get_flyby_bounds(planet_name):
    ...
    p = math.pi
    if planet_name == 'earth': rp_bounds = [1.045, 5.0]
    elif planet_name == 'venus': rp_bounds = [1.1, 5.0]
    elif planet_name == 'mars': rp_bounds = [1.1, 4.0]
    else: rp_bounds = [1.0, 10.0]
    return [-p, p], rp_bounds


def get_time_cap_days(tof_bounds):
    """公平总时长帽: min(自身下界和×倍数, 绝对封顶)"""
    lb_sum = sum(b[0] for b in tof_bounds)
    return min(FAIR_CAP_RATIO * lb_sum, FAIR_CAP_MAX_YEARS * 365.25)


def joi_capture_dv_kms(vinf_kms, rp_radii=None, period_days=None):
    """到达超速 → 近木点 JOI 捕获烧(km/s), Oberth 效应使捕获很便宜.
    校验: v∞=5.6, rp=1.06R_J, 捕获53.5天 → 0.539 (朱诺实测 542 m/s) ✓"""
    ...
    a = (MU_J * (period_days * 86400.0 / (2.0 * math.pi)) ** 2) ** (1.0/3.0)  # 捕获轨道半长轴
...
class DynamicMGA_UDP:
    def __init__(self, seq, t0_bounds, tof_bounds, vinf_bounds, time_cap):
        ...
        # —— 计费探针: 同一决策变量/边界, 仅 pykep 计费开关不同 → 用差分剥离各分量 ——
        #   f_full   = 出发v∞ + ΣDSM + 到达v∞   (dep=T, arr=T, 即主 udp)
        #   f_noarr  = 出发v∞ + ΣDSM            (dep=T, arr=F)
        #   f_novinf = ΣDSM                     (dep=F, arr=F)
        ...
        try:
            ...
            self._p_novinf = mga_1dsm(seq=seq, t0=t0_bounds, tof=tof_bounds,
                                      ...
                                      add_vinf_arr=False, multi_objective=False)
            nx0 = pg.problem(self._udp).get_nx()                              # 探针必须与主 udp
            nx1 = pg.problem(self._p_noarr).get_nx()                          # 决策变量完全同构,
            nx2 = pg.problem(self._p_novinf).get_nx()                         # 否则差分无意义
            self._decompose_ok = (nx0 == nx1 == nx2)
        except Exception:
            self._decompose_ok = False                                        # 回退旧口径

    def __getattr__(self, name):
        if name.startswith('_'):
            raise AttributeError(name)
        return getattr(self._udp, name)
    ...
    def fitness(self, x):
        ...
        total = f_full
        # 2) 任务口径: 到达超速按 JOI 捕获烧计费(差分剥离), 发射可按需交给火箭
        if ARRIVAL_MODE == 'capture' and self._decompose_ok:
            ...
            joi_ms = 1000.0 * joi_capture_dv_kms(arr / 1000.0)                # 捕获烧(m/s)
            if CHARGE_DEPARTURE:
        ...
        if RESONANCE_MODE == 'soft':
            for i in self._resonant_legs:
                ...
                dist = min(abs(T - k * period) - resonance_tol(k, period)
                           for k in RESONANCE_HARMONICS)
                if dist > 0:
                    total += RESONANCE_SOFT_WEIGHT * dist
    ...
    def get_bounds(self):
        ...
        lb, ub = list(lb), list(ub)
        for i, planet_name in enumerate(self._seq_names[1:-1]):
            beta_bounds, rp_bounds = get_flyby_bounds(planet_name)
            b_idx, r_idx = self._flyby_slots[i]
            lb[b_idx], ub[b_idx] = beta_bounds
            lb[r_idx], ub[r_idx] = rp_bounds
        return lb, ub
...
def pick_champion(fs, xs):
    """多解选优(备用工具)"""
    best_i, best_f = 0, None
...
def build_udp(seq_names):
    ...
    for i in range(len(seq_names) - 1):
        ...
        bounds = get_tof_bounds(p1, p2, is_final)
        if p1 == p2 and not is_final and RESONANCE_MODE == 'hard':            # 共振剪枝仅 hard 模式
            narrowed = get_resonant_tof_bounds(p1, bounds)
            if narrowed is not None:
                bounds = narrowed
        tof_bounds.append(bounds)
    cap = get_time_cap_days(tof_bounds)                                       # 逐序列公平帽
    tof_bounds[-1][1] = min(tof_bounds[-1][1],                              # 末段上界按帽收紧
                            cap - sum(b[0] for b in tof_bounds[:-1]))
...
def optimize_sequence_worker(task):
    """worker: 多种子 SADE + COMPASS 精修 + 任务口径分解"""
    ...
    try:
        ...
        if best_f1 >= INFEASIBLE_THRESHOLD:                                   # 全败 → 追加种子
...
def report_progress(current, total, elapsed, seq_str, best_dv, failed=False):
    ...
    try:
        line = line[:os.get_terminal_size().columns - 5]
    except Exception:
        line = line[:115]
...
def main():
    ...
    if ARRIVAL_MODE == 'capture':
        ...
        print(f"          到达超速不再全额计费; 出发上限 {DEPARTURE_VINF_MAX} km/s "
              f"(C3≈{DEPARTURE_VINF_MAX ** 2:.0f}, 朱诺实况)")
    else:
        print(f"ΔV 口径: 全口径 = 出发v∞ + ΣDSM + 到达v∞ (旧口径)")
    _probe = build_udp(['earth', 'mars', 'jupiter'])                          # 开跑前体检探针
    if ARRIVAL_MODE == 'capture' and not _probe._decompose_ok:
        print("⚠ 计费探针在当前 pykep 版本不可用, 已自动回退为到达全额计费口径")
    print(f"扫描阶段: 每序列 {len(SCAN_SEEDS)} seeds; "
          f"精算阶段: Top-{TOP_N_REFINE} 序列 x {len(RESTART_SEEDS)} seeds")
    print(f"每序列: SADE({SADE_GEN}代) + COMPASS精修({COMPASS_FEVALS}次)")
    print(f"ΔV 换算系数 DV_TO_KMS = {DV_TO_KMS}\n")

    t_start = time.time()

    print("=== 阶段一: 全序列扫描 ===\n")
    scan_tasks = [(names, SCAN_SEEDS) for names in sequences]
    results, scan_errors = run_pool(scan_tasks, t_start)
    ...
    if results:
        ...
        if top:
            ...
            refine_tasks = [(names, RESTART_SEEDS) for names in top]
            refined, refine_errors = run_pool(refine_tasks, time.time())
            refined_map = {tuple(r[4]): r for r in refined}
            results = [refined_map.get(tuple(r[4]), r) for r in results]

    t_total = time.time() - t_start

    all_errors = scan_errors + [(f"[精算]{s}", e) for s, e in refine_errors]
    if all_errors:
        print("\n" + "=" * 50)
        print(f"⚠ 捕获到 {len(all_errors)} 个任务失败:")
        for s, e in all_errors:
            print(f"  [{s}]: {e}")

    results.sort(key=lambda r: r[2])
    print(f"\n总耗时: {t_total:.1f} 秒 ({t_total/60:.1f} 分钟)")
    print("\n" + "=" * 110)
    print(f"              {total} 种序列 ΔV 排名表 (任务口径)")
    print("=" * 110)
    ...
    for rank, (seq_str, f1, f2, _, _, _, bd) in enumerate(results, 1):
        ...
        print(f"{rank:<5} | {seq_str:<30} | {dv1_km:<12.3f} "
              f"| {dv2_km:<12.3f} | {dep_s:<10} | {sc_s:<10} | {status_of(f1, f2)}")

    for rank, (s, _) in enumerate(scan_errors, len(results) + 1):
        print(f"{rank:<5} | {s:<30} | {'N/A':<12} | {'N/A':<12} "
              f"| {'N/A':<10} | {'N/A':<10} | ✗ 失败")

    print("-" * 110)
    print("总ΔV = 出发v∞ + ΣDSM + JOI捕获;  '出发(火箭)'列可由运载火箭实现(C3=v∞²), "
          "不消耗航天器推进剂")
    print("航天器推进 = ΣDSM + JOI, 才是探测器推进系统需承担的量; "
          "到达超速已按 JOI 捕获烧计费(Oberth 效应)")
    ...
    if results and results[0][3] is not None \
        ...
        if bd:
            ...
            print(f"    最优 ΔV = {best[2] * DV_TO_KMS:.3f} km/s (旧口径)")
        try:
